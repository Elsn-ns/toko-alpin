...
<div class="space-y-12 py-4">
    ...
    <div class="max-w-2xl">
        <h1 class="text-4xl font-black tracking-tight text-slate-900 leading-tight">
            Support <span class="text-indigo-600">Terminal</span>
        </h1>
        <p class="text-slate-500 mt-2 text-lg font-medium">Manage customer communications and resolve pending tickets.</p>
    </div>

    <div class="flex flex-col lg:flex-row gap-10 min-h-[700px]">
        <!-- Left: Conversation Selection -->
        <div class="w-full lg:w-[400px] shrink-0">
            <div class="glass-card flex flex-col h-[700px] border border-white shadow-xl shadow-slate-200/50 overflow-hidden bg-white/70 backdrop-blur-xl">
                <div class="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                    <h3 class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Active Streams</h3>
                    <span class="badge-modern bg-indigo-50 text-indigo-600 border border-indigo-100">{{ $conversations->count() }} Files</span>
                </div>
                ...
                <div class="flex-1 overflow-y-auto custom-scrollbar">
                           ...
                           class="block p-8 border-b border-slate-50 transition-all duration-300 group {{ isset($conversation) && $conversation->id === $conv->id ? 'bg-indigo-50 border-l-4 border-l-indigo-600 shadow-inner' : 'hover:bg-slate-50' }}">
                            <div class="flex justify-between items-start">
                                <div class="space-y-2 max-w-[200px]">
                                    <p class="font-black text-slate-900 group-hover:text-indigo-600 transition-colors tracking-tight truncate">{{ $conv->customer->name ?? 'Guest User' }}</p>
                                    <p class="text-xs font-medium text-slate-400 truncate">{{ $conv->messages->first()->body ?? 'Initialize stream...' }}</p>
                                </div>
                    ...
                    @empty
                        <div class="flex flex-col items-center justify-center h-full p-10 text-center space-y-4 opacity-30">
                            ...
                            <p class="text-[10px] font-black uppercase tracking-widest text-slate-500">Zero active streams</p>
                        </div>
                    @endforelse
                </div>
            </div>
        ...
        <!-- Right: Chat Window -->
        <div class="flex-1">
            <div class="glass-card flex flex-col h-[700px] border border-white shadow-2xl shadow-slate-200/40 overflow-hidden bg-white">
                @if(isset($conversation))
                    <div class="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                        <div class="flex items-center gap-5">
                            ...
                            <div>
                                <h2 class="text-xl font-black text-slate-900 tracking-tight">{{ $conversation->customer->name ?? 'Guest User' }}</h2>
                                <div class="flex items-center gap-2 mt-0.5">
                                    <div class="w-2 h-2 bg-emerald-500 rounded-full"></div>
                                    <span class="text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em]">Stream Online</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <form action="{{ route('chat.send') }}" method="POST" class="flex items-end gap-6">
                            ...
                            <div class="flex-1">
                                <textarea name="body" required rows="1" 
                                          ...
                                          class="input-modern !py-5 !px-8 !rounded-[32px] resize-none overflow-hidden"
                ...
                @else
                    <div class="flex-1 flex flex-col items-center justify-center text-center p-20 space-y-6 opacity-40">
                        ...
                        <div class="space-y-2">
                            <h3 class="text-2xl font-black text-slate-900 tracking-tight">Select Node</h3>
                            <p class="text-sm font-medium text-slate-500">Initialize a communication stream to interact with clients.</p>
                        </div>
                    </div>
                @endif
            </div>
