    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
        <div class="max-w-xl">
            <h1 class="text-4xl font-black tracking-tight text-slate-900 leading-tight">
                Inventori <span class="text-indigo-600">Produk</span>
            </h1>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100 text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">
                        <th class="px-8 py-6">Item Produk</th>
                        <th class="px-8 py-6">Klasifikasi</th>
                        <th class="px-8 py-6 text-right">Valuasi</th>
                        <th class="px-8 py-6 text-center">Ketersediaan</th>
                        <th class="px-8 py-6 text-center">Operasi</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-slate-50/50 transition-all duration-300 group">
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-5">
                                    <div class="w-14 h-14 rounded-2xl bg-white flex-shrink-0 flex items-center justify-center font-black text-slate-400 text-xl border border-slate-200/30 group-hover:bg-indigo-600 group-hover:text-white group-hover:rotate-3 transition-all duration-500 overflow-hidden shadow-sm">
                                        @if($product->image)
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">KODE: {{ $product->product_code }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                            ...
                            <td class="px-8 py-6 text-center">
                                <div class="flex flex-col items-center gap-1">
                                    ...
                                    <span class="text-[10px] font-black uppercase tracking-widest text-slate-300">Stok</span>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                                <div class="flex justify-center items-center gap-2">
                                    @if(auth()->user()->role === 'admin')
                                        ...
                                        <form action="{{ route('products.destroy', $product) }}" method="POST" onsubmit="return confirm('Arsip produk ini?')">
                                            @csrf @method('DELETE')
                                            <button type="submit" 
                                    ...
                                    @else
                                        <span class="text-[10px] font-black uppercase text-slate-300 italic tracking-widest">Read Only</span>
                                    @endif
                                </div>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td colspan="5" class="px-8 py-32 text-center">
                                <div class="max-w-xs mx-auto space-y-4">
                                    ...
                                    <p class="text-slate-500 text-sm">Mulai tambahkan produk ke toko Anda untuk mengaktifkan pelacakan penjualan dan inventori.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
